...
def ads(target='https://vk.com/kirucha_apastnost'):
    try:
...
class AntichristApp(MDApp):
    def __init__(self, **kwargs):
        ...
        if self.start_hell == None:
            ...
            if [self.configuration_screen['my_style'][0], self.storage] == ['True', True]:
                ...
                if os.path.isfile(photo) == True:
                    ...
                    start_my_style=start_my_style.replace('text: "[color=#fedcff]CONTENT[/color]"' ,f'text: "[color={self.title}]CONTENT[/color]"')
                    ...
                    start_my_style=start_my_style.replace("text: '[color=#fedcff]CONTENT[/color]'", f"text: '[color={self.link_tg_color}]CONTENT[/color]'")
        ...
        self.open = 'https://vk.com/kirucha_apastnost'
    ...
    def welcom_to_hell_activity(self, action=None):
        if action == 'hell':
            ...
            if [self.configuration_screen['my_style'][0], self.storage] == ['True', True]:
                if self.title == None:
                    self.screen.ids.title.text = f"[color=#fedcff]CONTENT[/color]"
                else:
                    self.screen.ids.title.text = f"[color={self.title}]CONTENT[/color]"
            else:
                self.screen.ids.title.text = f"[color=#fedcff]CONTENT[/color]"
